...
plot_pomdp_fsp.py
=================
Three diagnostic plots for the POMDP section, using the FSP Nash-equilibrium
mixed strategy as the frozen P2 opponent. Run from the code/ directory.

Saves:
  pomdp_obs_levels_fsp.png  — QMDP win rate vs. observation granularity
  pomdp_entropy_fsp.png     — belief entropy over a game (mean + individual traces)
  pomdp_mismatch_fsp.png    — QMDP vs. POMCP under model mismatch
"""
...
from policy import load_mixed_policy, FunctionPolicy
...
# Load FSP mixed P2 policy
_tmp   = POMDPEnv(opponent_policy=lambda s: Action.P2_TRAIN_MARINES,
                  initial_state=S_INIT, n_obs_levels=1)
fsp_p2 = load_mixed_policy('fsp_sigma_p2.npy', _tmp.S, Action.P2_ACTIONS)
...
print("VI solve 1/2: FSP P2...")
env_det = POMDPEnv(opponent_policy=fsp_p2,
                   initial_state=S_INIT, n_obs_levels=1)
...
for n in levels:
    env_n = POMDPEnv(opponent_policy=fsp_p2,
...
ax1.set_ylabel('Rate')
ax1.set_title('QMDP win rate vs. observation granularity (FSP opponent)')
ax1.set_ylim(0, 1)
ax1.legend()
fig1.tight_layout()
fig1.savefig('pomdp_obs_levels_fsp.png', dpi=150)
print("Saved pomdp_obs_levels_fsp.png")
...
env_e = POMDPEnv(opponent_policy=fsp_p2,
...
ax2.set_title('Belief entropy over a game (QMDP, n_obs_levels=4, FSP opponent)')
ax2.legend()
fig2.tight_layout()
fig2.savefig('pomdp_entropy_fsp.png', dpi=150)
print("Saved pomdp_entropy_fsp.png")
...
ax3.set_ylabel('Rate')
ax3.set_title('QMDP vs. POMCP under model mismatch (FSP opponent)')
ax3.set_ylim(0, 1)
ax3.legend()
fig3.tight_layout()
fig3.savefig('pomdp_mismatch_fsp.png', dpi=150)
print("Saved pomdp_mismatch_fsp.png")
